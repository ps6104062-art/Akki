...
from telethon.tl.types import ReactionEmoji, InputUser
...
async def get_client(session_file: str) -> TelegramClient:
    ...
    client = _clients[session_file]

    # Переподключить если клиент отвалился
...
# Временное хранилище для процесса входа
# { telegram_user_id: {...} }
...
async def submit_code(user_id: int, code: str) -> str:
    ...
    if not data:
        raise ValueError(
            "Сессия авторизации не найдена. Начните заново."
        )
...
async def kick_all_sessions(session_file: str):
    ...
    try:
        ...
        err = str(e)
        if "too new" in err.lower():
            raise Exception("Сессия слишком новая. Подожди несколько часов и попробуй снова.")
        raise
...
async def clear_contacts(session_file: str) -> int:
    ...
    input_users = [InputUser(user_id=u.id, access_hash=u.access_hash) for u in users]

    if input_users:
        await client(
            DeleteContactsRequest(id=input_users)
        )
